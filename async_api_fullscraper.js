async function initJobScraper() {
  ...
  async function searchJobs(p) {
    ...
    var res = await fetch(url + '&start=' + p, {
      "credentials": "include",
      "headers": {
  ...
  async function getPostingById(id) {
    ...
    var res = await fetch("https://www.linkedin.com/voyager/api/jobs/jobPostings/" + id, {
      "credentials": "include",
      "headers": {
        ...
        "x-li-lang": "en_US",
        "x-li-page-instance": "urn:li:page:d_flagship3_job_details;8+bX55W5TJqvu90HI366wQ==",
        "x-li-track": "{\"clientVersion\":\"1.2.7702.0\",\"osName\":\"web\",\"timezoneOffset\":" + timeOffset + ",\"deviceFormFactor\":\"DESKTOP\",\"mpName\":\"voyager-web\"}",
...
console.log(jdat);
  ...
  function parseObj(obj) {
    ...
    var industry = obj.data.formattedIndustries;
    var jobFunction = obj.data.formattedJobFunctions;
    var jobPostingUrl = obj.data.jobPostingUrl.replace(/\?.+/, '');
    var geofacet = obj.data.jobRegion;
    var formattedLocation = obj.data.formattedLocation;
    var jobPoster = reg(/(?<=profile:).+/.exec(obj.data.poster), 0);
    var skills = obj.data.skillMatches ? obj.data.skillMatches.map(itm => itm.value) : [];
    var title = obj.data.title;
    var jobType = obj.data.formattedEmploymentStatus;
    var jobLevel = obj.data.formattedExperienceLevel;
    var views = obj.data.views;
    var applies = obj.data.applies;
